...
export default function ArtistPage() {
        ...
        return (
            <div className="flex h-screen bg-[#050505] text-white">
                ...
                <main className="flex-1 p-8 flex justify-center items-center">
                    <p className="text-xl text-gray-400 italic font-serif">Artist not found in the archive</p>
                </main>
            </div>
        );
    }
    ...
    return (
        <div className="flex h-screen bg-[#050505] text-white selection:bg-retro-gold font-sans overflow-hidden">
            <Sidebar />
            <main className="flex-1 overflow-y-auto">
                {/* Hero Section */}
                <div className="relative h-[450px] bg-[#0a0a0a] flex items-end">
                    {/* Background Image with Blur */}
                    {artist.image_url && (
                        <div
                            className="absolute inset-0 opacity-30 grayscale blur-xl"
                    ...
                    <div className="relative z-10 p-12 md:p-16 flex items-end gap-10 w-full max-w-[1600px] mx-auto">
                        <div className="w-64 h-64 rounded-full overflow-hidden shadow-[0_0_50px_rgba(0,0,0,1)] border-4 border-white/5 shrink-0 bg-gray-900 flex items-center justify-center">
                            {artist.image_url && artist.image_url.trim() !== '' ? (
                                <img
                                    ...
                                    onError={(e) => {
                                        ...
                                        if (parent) {
                                            ...
                                            fallback.className = "w-full h-full bg-gray-900 flex items-center justify-center text-8xl";
                                            fallback.innerText = '🎤';
                                ...
                                <div className="w-full h-full bg-gray-900 flex items-center justify-center text-8xl">🎤</div>
                            )}
                        </div>
                        <div className="mb-4 flex-1">
                            <div className="flex items-center gap-3 mb-4">
                                <div className="w-2 h-2 rounded-full bg-retro-gold" />
                                <span className="text-[10px] uppercase tracking-[0.4em] font-bold text-retro-gold italic">Verified Visionary</span>
                            </div>
                            <h1 className="text-7xl md:text-9xl font-serif italic leading-none tracking-tighter mb-8">{artist.name}</h1>

                            <div className="flex items-center gap-6 text-gray-400 mb-8 font-light italic">
                                <span>{artist.genres?.join(', ') || 'Genre Underground'}</span>
                                {artist.popularity > 0 && (
                                    <>
                                        <div className="w-1 h-1 rounded-full bg-white/20" />
                                        <span className="text-retro-gold font-bold">{artist.popularity}% Archives Resonance</span>
                                    </>
                                )}
                            </div>
                            ...
                            <button
                                ...
                                className={`px-12 py-4 rounded-none font-bold text-[10px] uppercase tracking-[0.3em] transition-all ${isFollowing
                                    ? 'bg-transparent border border-white/20 text-white hover:border-white'
                                    : 'bg-white text-black hover:bg-retro-gold hover:text-black shadow-2xl'
                                    }`}
                            >
                ...
                <div className="p-8 md:p-16 max-w-[1600px] mx-auto">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-8 border-b border-white/5 pb-8">
                        <div>
                            <h2 className="text-5xl font-serif italic text-white mb-2 underline decoration-retro-gold/20 underline-offset-8">Discography</h2>
                            <div className="flex items-center gap-3 mt-4">
                                <div className="w-8 h-[1px] bg-retro-gold/50" />
                                <p className="text-[9px] uppercase tracking-widest text-gray-500 font-bold">Curated Records</p>
                            </div>
                        </div>
                        ...
                        <div className="relative w-full md:w-80">
                            <input
                                ...
                                className="w-full bg-white/[0.03] border border-white/10 px-6 py-4 text-[10px] tracking-widest uppercase focus:outline-none focus:border-retro-gold/50 transition-colors"
                            />
                            <span className="absolute right-6 top-1/2 -translate-y-1/2 text-gray-600">🔍</span>
                        </div>
                    </div>
